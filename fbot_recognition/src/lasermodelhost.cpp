...
namespace {
	...
	std::string resolvePackagePath(std::string const& path)
	{
		std::string result = path;
		size_t find_pos = result.find("$(find ");
		if (find_pos != std::string::npos) {
			size_t start_pos = find_pos + 7; // length of "$(find "
			size_t end_pos = result.find(")", start_pos);
			if (end_pos != std::string::npos) {
				std::string package_name = result.substr(start_pos, end_pos - start_pos);
				std::string package_path = ament_index_cpp::get_package_share_directory(package_name);
				result.replace(find_pos, end_pos - find_pos + 1, package_path);
			}
		}
		return result;
	}
...
public:
	...
	LaserModel(rclcpp::Logger log, Ort::Env& env, std::string const& modelPath, float near, float far, float thresh, bool useCuda) :
		m_log{log}, m_scanNear{near}, m_scanFar{far}, m_scoreThresh{thresh}
	{
		...
		if (useCuda) {
			...
			RCLCPP_INFO(log, "Using CUDA execution provider for ONNX inference");
		} else {
			// Use CPU execution provider
			options.SetExecutionMode(ExecutionMode::ORT_SEQUENTIAL);
			options.DisableMemPattern();
			options.DisableCpuMemArena();
			RCLCPP_INFO(log, "Using CPU execution provider for ONNX inference");
		}
...
public:
	...
	{
		auto model_file_raw = declare_parameter<std::string>("model_file");
		auto model_file = resolvePackagePath(model_file_raw);

		auto laser_topic  = declare_parameter<std::string>("laser_topic",  "/scan");
		...
		auto use_cuda = declare_parameter<bool>("use_cuda",       false);

		RCLCPP_DEBUG(get_logger(), "near=%f far=%f", near, far);

		m_model.emplace(get_logger(), m_ortEnv, model_file, near, far, thresh, use_cuda);
		...
		RCLCPP_INFO(get_logger(), "LaserModelHost initialized. model_file=%s, subscribing to %s, publishing %s and %s",
			model_file.c_str(), laser_topic.c_str(), output_topic.c_str(), marker_topic.c_str());
	}
...
RCLCPP_COMPONENTS_REGISTER_NODE(laser_people_detector::LaserModelHost)
